...
Google API 직접 호출 + Ollama 로컬 LLM 기반.
...
sequenceDiagram
    ...
    participant Matcher as 패턴 매칭 (Python)
    ...
    Note over Watcher: Phase 1: 제목+발신자만 (LLM 미사용)
    Watcher->>Memory: 메모리 컨텍스트 로드
    Watcher->>Matcher: 발신자/키워드 패턴 매칭
    Matcher-->>Watcher: fast / need_body 분류

    loop fast 항목
        Watcher->>Gmail: 라벨 + 보관 + _processed
    end

    Note over Watcher: Phase 2: 본문 포함 (LLM 사용)
    loop need_body 항목
        Watcher->>Gmail: 스레드 상세 조회
        Watcher->>LLM: 개별 분류 요청
        alt 자동 분류
            Watcher->>Gmail: 라벨 + 보관 + _processed
        else 미결정
...
graph LR
    ...
    subgraph "일간 통합"
        MC[memory-consolidator<br/>Ollama LLM]
    end
...
- Phase 2에서 스레드 전체 대화를 LLM에 전달 → 답장 내용까지 분석
